...
import ApproveItineraryModal from "./ApproveItineraryModal.jsx";
...
function parseRunTargetKey(key) {
  ...
  return createPlanningContext(type, id);
}
...
async function createDraftThreadState({
  agencyId,
...
export default function HomePage({ user: userProp, agencyTrips = [], onContinue, onOpenTrip, onNewItinerary }) {
  ...
  const [activeContext, setActiveContext] = useState(() => createPlanningContext("trip", agencyTrips[0]?.id ?? null));
  ...
  const draftThreadOrderRef = useRef(draftThreadOrder);
  const isCreatingDraftThreadRef = useRef(false);
  ...
  useEffect(() => {
    draftThreadOrderRef.current = draftThreadOrder;
  }, [draftThreadOrder]);

  useEffect(() => {
    isCreatingDraftThreadRef.current = isCreatingDraftThread;
  ...
  useEffect(() => {
    ...
    if (activeContext?.type === "draft") {
      return;
    }

    const hasCurrentSelection = agencyTrips.some((trip) => trip?.id === activeContext?.id);
    if (!hasCurrentSelection) {
      setActiveContext(createPlanningContext("trip", agencyTrips[0]?.id ?? null));
    }
  ...
  useEffect(() => {
    ...
    if (runTarget.type === "draft") {
      setDraftThreadStates((previous) => {
        const current = previous[runTarget.id] ?? {
          threadId: runTarget.id,
          title: "",
          tripId: null,
          messages: [],
          itinerary: null,
  ...
  const activeTrip = useMemo(() => {
    ...
    return safeTrips.find((trip) => trip?.id === activeContext.id) ?? null;
  }, [activeContext, safeTrips]);
  const activeTripState =
    activeContext?.type === "draft"
      ? draftThreadStates[activeContext.id] ?? null
      : activeTrip?.id
        ? tripStates[activeTrip.id] ?? null
        : null;
  const planningOptions = useMemo(() => {
    const draftOptions = draftThreadOrder
      .map((threadId, index) => {
        const state = draftThreadStates[threadId];
        if (!state) return null;

        const itineraryTrip = state.itinerary?.trip ?? {};
        const draftLabel = state.title || `Draft itinerary ${draftThreadOrder.length - index}`;
        const destination =
          itineraryTrip.destinationSummary ||
          itineraryTrip.destination ||
          itineraryTrip.title ||
    ...
    const tripOptions = safeTrips.map((trip) => ({
      type: "trip",
      id: trip.id,
      clientName: trip.clientName,
      label: trip.clientName,
      destination: trip.destination,
      statusLabel: trip.approvalStatus || trip.status || "Client trip",
      tripId: trip.id,
      threadId: tripStates[trip.id]?.threadId ?? null,
      assignedOrganizer: trip.assignedOrganizer,
    }));

    return [...draftOptions, ...tripOptions];
  }, [draftThreadOrder, draftThreadStates, safeTrips, tripStates]);
  const activeOption = useMemo(() => {
    if (!activeContext) return planningOptions[0] ?? null;
    return planningOptions.find((option) => option.type === activeContext.type && option.id === activeContext.id) ?? null;
  }, [activeContext, planningOptions]);
  ...
  const isLiveStreamVisible = Boolean(activeContextKey && activeContextKey === runTargetRef.current);
  const scopedIsStreaming = isLiveStreamVisible ? isStreaming : false;
  const scopedRunStatus = isLiveStreamVisible ? runStatus : "idle";
  const scopedAssistantMessage = isLiveStreamVisible ? assistantMessage : "";
  const scopedToolCalls = isLiveStreamVisible ? toolCalls : [];
  const scopedMapMarkers = isLiveStreamVisible ? mapMarkers : [];
  const scopedRouteEstimates = isLiveStreamVisible ? routeEstimates : [];
  const scopedActiveToolLabel = isLiveStreamVisible ? activeToolLabel : null;
  const scopedStreamError = isLiveStreamVisible ? streamError : null;
  const liveStatus = getRunStatusLabel(scopedRunStatus, scopedStreamError);
  ...
  async function dispatchAgentMessage(rawPrompt) {
    ...
    try {
      ...
      if (!currentContext) {
        ensuredState = await createDraftThreadState({
          agencyId,
          setDraftThreadStates,
          setDraftThreadOrder,
        });
        ...
        ensuredState = await ensureTripThreadState({
          agencyId,
          tripId: currentContext.id,
          tripStatesRef,
          tripStatePromisesRef,
          setTripStates,
        });
      ...
      const currentThreadId =
        ensuredState?.threadId ??
        (currentContext?.type === "draft"
          ? draftThreadStatesRef.current[currentContext.id]?.threadId
          : tripStatesRef.current[currentContext?.id]?.threadId) ??
        null;
      ...
      if (currentContext.type === "draft") {
        setDraftThreadStates((previous) => {
          const current = previous[currentContext.id] ?? {
  ...
  async function handleNewItinerary() {
    if (isCreatingDraftThreadRef.current) {
      return;
    }
    ...
    setAgentError("");
    setComposerInput("");
    isCreatingDraftThreadRef.current = true;
    setIsCreatingDraftThread(true);

    try {
      const nextState = await createDraftThreadState({
        agencyId,
        setDraftThreadStates,
        setDraftThreadOrder,
      });
      setActiveContext(createPlanningContext("draft", nextState?.threadId ?? null));
    } catch (error) {
      ...
      isCreatingDraftThreadRef.current = false;
  ...
  async function submitDraftApproval(fields) {
    ...
    const threadId = activeContextRef.current.id;
    const draftState = draftThreadStatesRef.current[threadId];
    const itineraryId = draftState?.itinerary?.id;

    if (!itineraryId) {
      setApprovalError("Generate an itinerary before saving this draft.");
      return;
    }
    ...
    try {
      const result = await approveAgentThreadItinerary(agencyId, threadId, {
        itineraryId,
        ...fields,
      });
      const approvedThread = result?.thread ?? null;
      const approvedTrip = result?.trip ?? null;
      const nextTitle = approvedThread?.title || approvedTrip?.clientName || fields.clientName;
      const nextTripId = approvedThread?.tripId || approvedTrip?.id || draftState.tripId;
      ...
      setApprovalError(error?.message || "Unable to save this draft to a client.");
    } finally {
            ...
            <section className="hero-stack">
              <div className="agentic-surface">
                <AgentCommandCenter
                  ...
                  toolCalls={scopedToolCalls}
                  activeToolLabel={scopedActiveToolLabel}
                ...
                <ItineraryDraftPanel
                  ...
                  mapMarkers={scopedMapMarkers}
                  routeEstimates={scopedRouteEstimates}
